...
static int
security_callback_allow_all(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other, void *ex) {
	return 1;
}
...
int populateCipherList(struct sslCheckOptions *options, const SSL_METHOD *sslMethod) {
	...
	for (loop = 0; loop < sk_SSL_CIPHER_num(cipherList); loop++) {
		...
		SSL_CIPHER_description(sk_SSL_CIPHER_value(cipherList, loop), sslCipherPointer->description,
		                       sizeof(sslCipherPointer->description) - 1);
...
char *sock_strerror(int err) {
	...
	if (FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK,
	                  NULL, err, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), msg, sizeof(msg), NULL) == 0 ||
	    msg[0] == '\0') {
...
int testHeartbleed(struct sslCheckOptions *options, const SSL_METHOD *sslMethod) {
	...
	if (socketDescriptor != 0) {
		...
		char hello[] = {0x16, 0x03, 0x01, 0x00, 0xdc, 0x01, 0x00, 0x00, 0xd8, 0x03, 0x00, 0x53, 0x43, 0x5b, 0x90, 0x9d,
...
void outputCipher(struct sslCheckOptions *options, SSL *ssl, const char *cleanSslMethod, uint32_t cipherid,
                  const char *ciphername, int cipherbits, int cipher_accepted, unsigned int milliseconds_elapsed) {
		...
		if (setCipherSuite(options, sslMethod, options->cipherstring)) {
			...
			if (ssl != NULL) {
				...
				if (cipherStatus == 1) {
					...
					if (sslMethod == TLSv1_3_client_method()) { // Remove cipher from TLSv1.3 list
...
*/
								...
								if (!(X509_FLAG_COMPAT & X509_FLAG_NO_EXTENSIONS)) {
									if (sk_X509_EXTENSION_num(X509_get0_extensions(x509Cert)) > 0) {
										...
										for (tempInt = 0; tempInt < sk_X509_EXTENSION_num(
												X509_get0_extensions(x509Cert)); tempInt++) {
											// Get Extension...
											extension = sk_X509_EXTENSION_value(X509_get0_extensions(x509Cert),
											                                    tempInt);
											...
											if (options->xmlOutput) {
												...
												BIO_printf(fileBIO, "\"%s><![CDATA[",
												           tempInt2 ? " level=\"critical\"" : "");
											}
...
int runSSLv3Test(struct sslCheckOptions *options) {
	...
	char sslv3_client_hello_2[] = {
			...
			0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, /* Random bytes */
...
bs *makeCiphersuiteListMissing(unsigned int tls_version) {
	...
	for (int i = 0; i < (sizeof(missing_ciphersuites) / sizeof(struct missing_ciphersuite)); i++) {
		...
		if ((missing_ciphersuites[i].check_tls_versions & tls_version) &&
		    ((missing_ciphersuites[i].accepted_tls_versions & tls_version) == 0)) {
			bs_append_ushort(ciphersuite_list, missing_ciphersuites[i].id);
		}
	}
...
bs *
makeClientHello(struct sslCheckOptions *options, unsigned int tls_version, bs *ciphersuite_list, bs *tls_extensions) {
...
int testMissingCiphers(struct sslCheckOptions *options, unsigned int tls_version) {
	...
	while (1) {
		...
		if ((bs_get_byte(server_hello, 1) != 0x03) ||
		    (bs_get_byte(server_hello, 2) != (unsigned char) tls_version_low_byte))
			goto done;
		...
		if ((session_id_len + 43 + 2 + 1) > bs_get_len(server_hello)) {
			fprintf(stderr,
			        "Error: size of server_hello (%"SIZE_T_FMT") is not large enough to reach cipher suite (%u).\n",
...
#define NID_TYPE_X448 4
	...
	for (index = 0; index < (sizeof(test_versions) / sizeof(int)); index++) {
		...
		if (tls_version == TLSv1_3)
			...
			bs_append_bytes(ciphersuite_list,
			                (unsigned char[]) {,
		...
		for (int i = 0; i < (sizeof(group_key_exchanges) / sizeof(struct group_key_exchange)); i++) {
			...
			if (tls_version < TLSv1_3) {
				...
				uint16_t server_group_id = bs_get_byte(tls_record, handshake_type_offset + 5) << 8 |
				                           bs_get_byte(tls_record, handshake_type_offset + 6);
			...
			printf("%s  %s%d%s bits  %s%s%s\n", printable_TLS_name, bits_color, group_bit_strength, RESET, color,
			       group_name, RESET);
...
void print_result_to_file(struct result *res) {
	...
	sprintf(buffer, "%d,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", res->index, res->host,
	        ...
	        (unsigned char) res->certificate_key_category, res->certificate_keyBits, \
            (unsigned char) res->certificate_issuer, (unsigned char) res->protocol_downgrade,
...
int main(int argc, char *argv[]) {
	...
	printf("output the final result\n");
	fp = fopen(outfile, "w");
	if (fp == NULL) {
		fprintf(stderr, "fopen() failed.\n");
		exit(EXIT_FAILURE);
